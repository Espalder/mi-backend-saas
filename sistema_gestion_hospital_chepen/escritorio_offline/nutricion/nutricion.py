...
class NutricionWindow:
    def __init__(self):
        self.root = tk.Toplevel()
        self.root.title("Nutrición")
    ...
    def setup_ui(self):
        ...
        tk.Label(self.root, text="Nutrición", font=fuente_titulo, bg="#f0f9ff", fg="#0ea5e9").pack(pady=(0, 10))
        frame = tk.Frame(self.root, bg="#f0f9ff")
        frame.pack(pady=5)
        tk.Label(frame, text="Paciente ID:", bg="#f0f9ff").grid(row=0, column=0, padx=5, pady=5)
        tk.Label(frame, text="Plan Alimentario:", bg="#f0f9ff").grid(row=0, column=2, padx=5, pady=5)
        ...
        tk.Entry(frame, textvariable=self.plan_var, width=30).grid(row=0, column=3, padx=5, pady=5)
        ...
        tk.Button(boton_frame, text="Agregar", command=self.agregar_plan, bg="#7dd3fc", fg="#166534", font=fuente_boton, width=12, activebackground="#0ea5e9").grid(row=0, column=0, padx=5)
        tk.Button(boton_frame, text="Actualizar", command=self.actualizar_plan, bg="#93c5fd", fg="#166534", font=fuente_boton, width=12, activebackground="#0ea5e9").grid(row=0, column=1, padx=5)
        tk.Button(boton_frame, text="Eliminar", command=self.eliminar_plan, bg="#fca5a5", fg="#166534", font=fuente_boton, width=12, activebackground="#991b1b").grid(row=0, column=2, padx=5)
        self.tree = ttk.Treeview(self.root, columns=("id", "paciente_id", "plan", "fecha"), show="headings")
        self.tree.heading("id", text="ID")
        self.tree.heading("paciente_id", text="Paciente ID")
        self.tree.heading("plan", text="Plan Alimentario")
        self.tree.heading("fecha", text="Fecha")
        self.tree.column("id", width=40)
        self.tree.column("paciente_id", width=80)
        self.tree.column("plan", width=200)
        self.tree.column("fecha", width=120)
        self.tree.pack(pady=10, fill=tk.X, padx=10)
        self.tree.bind("<ButtonRelease-1>", self.seleccionar_plan)
    ...
    def agregar_plan(self):
        ...
        if not paciente_id or not plan:
            messagebox.showwarning("Validación", "Paciente ID y Plan son obligatorios.")
            return
        conn = self.conectar()
        c = conn.cursor()
        c.execute("INSERT INTO nutricion (paciente_id, plan, fecha) VALUES (?, ?, ?)",
                  (paciente_id, plan, fecha))
        conn.commit()
        conn.close()
        self.limpiar_campos()
        self.load_planes()
    ...
    def actualizar_plan(self):
        ...
        conn = self.conectar()
        c = conn.cursor()
        c.execute("UPDATE nutricion SET paciente_id=?, plan=?, fecha=? WHERE id=?",
                  (paciente_id, plan, fecha, self.selected_id))
        conn.commit()
        conn.close()
        self.limpiar_campos()
        self.load_planes()
    ...
    def eliminar_plan(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Está seguro de eliminar este registro?"):
            conn = self.conectar()
            c = conn.cursor()
            c.execute("DELETE FROM nutricion WHERE id=?", (self.selected_id,))
            conn.commit()
            conn.close()
            self.limpiar_campos()
            self.load_planes()
    ...
    def load_planes(self):
        ...
        conn = self.conectar()
        c = conn.cursor()
        c.execute("SELECT id, paciente_id, plan, fecha FROM nutricion")
        for row in c.fetchall():
            self.tree.insert("", tk.END, values=row)
        conn.close()
        self.selected_id = None

    def seleccionar_plan(self, event):
