...
class ReportesUI:
    def __init__(self, parent, conn, cursor, rol, tema='claro'):
        ...
        self.graficos = GraficosEstadisticas(self.parent, self.conn, self.cursor, self.tema)
    ...
    def crear_interfaz(self):
        ...
        # Limpiar cuando se cambia de pestaña y redibujar si hay datos
        def on_tab_change(event):
            tab_text = self.notebook.tab(self.notebook.select(), 'text')
            if tab_text == "📊 Gráfico Barras" and self.datos_actuales['ventas']:
                for w in self.frame_grafico_barras.winfo_children():
                    w.destroy()
                self.generar_graficos_reporte(self.datos_actuales['ventas'], self.datos_actuales['productos'])
            elif tab_text == "🥧 Gráfico Torta" and self.datos_actuales['productos']:
                for w in self.frame_grafico_torta.winfo_children():
                    w.destroy()
                self.generar_graficos_reporte(self.datos_actuales['ventas'], self.datos_actuales['productos'])
    ...
    def generar_pdf(self):
        ...
        try:
            ...
            # Convertir datos a dicts si vienen como tuplas (id, fecha, cliente, total)
            ventas_para_pdf = []
            for v in self.datos_actuales['ventas']:
                if isinstance(v, (list, tuple)):
                    ventas_para_pdf.append({
                        'id': v[0],
                        'fecha': str(v[1]),
                        'cliente_nombre': v[2],
                        'total': float(v[3]),
                    ...
                    ventas_para_pdf.append(v)

            # Generar el PDF (usar firma real del generador)
