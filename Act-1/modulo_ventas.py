...
class VentasUI:
    ...
    def actualizar_precio_producto(self, event=None):
        ...
        try:
            ...
            if not codigo:
                return
            if self.conn.__class__.__module__.startswith('sqlite3'):
                query = "SELECT precio, stock FROM productos WHERE codigo = ? AND activo = 1"
                self.cursor.execute(query, (codigo,))
            else:
                query = "SELECT precio, stock FROM productos WHERE codigo = %s AND activo = TRUE"
                self.cursor.execute(query, (codigo,))
    ...
    def agregar_producto_venta(self):
        ...
        try:
            if self.conn.__class__.__module__.startswith('sqlite3'):
                query = "SELECT stock, nombre, id FROM productos WHERE codigo = ? AND activo = 1"
                self.cursor.execute(query, (codigo,))
            else:
    ...
    def finalizar_venta(self):
        ...
        if not messagebox.askyesno("Confirmar venta", f"¿Desea finalizar la venta?\n\nCliente: {cliente}\nTotal: S/{total:.2f}"):
            return
        try:
            # Manejar transacción para MySQL; SQLite hace autocommit por defecto
            try:
                if hasattr(self.conn, 'in_transaction') and self.conn.in_transaction:
                    self.conn.rollback()
                if hasattr(self.conn, 'start_transaction'):
            ...
            if cliente and cliente != "Consumidor Final":
                if self.conn.__class__.__module__.startswith('sqlite3'):
                    query_cliente = "SELECT id FROM clientes WHERE nombre = ?"
                ...
                if resultado_cliente:
                    ...
                    if self.conn.__class__.__module__.startswith('sqlite3'):
                        query_crear_cliente = "INSERT INTO clientes (nombre) VALUES (?)"
            ...
            if self.conn.__class__.__module__.startswith('sqlite3'):
                query = "INSERT INTO ventas (fecha, cliente_id, usuario_id, subtotal, descuento, total, estado) VALUES (?, ?, ?, ?, ?, ?, ?)"
            ...
            for item in items:
                ...
                if len(valores) >= 4:
                    ...
                    subtotal = float(valores[3])
                    if self.conn.__class__.__module__.startswith('sqlite3'):
                        query_producto = "SELECT id FROM productos WHERE codigo = ?"
                    ...
                    if cantidad > stock_actual:
                        ...
                        return
                    if self.conn.__class__.__module__.startswith('sqlite3'):
                        query = "INSERT INTO detalle_ventas (venta_id, producto_id, cantidad, precio_unitario, subtotal) VALUES (?, ?, ?, ?, ?)"
    ...
    def refrescar_automatico(self):
        self.parent.after(60000, self.cargar_ventas)
